 ...
 *   GET /callback → 用 code 换 access_token，重定向回 CMS 同源 callback.html
 *
 * 环境变量：
...
export default {
  async fetch(request, env) {
    ...
    // GET /callback — GitHub 回调，换 token 后重定向到 CMS 同源页面
    if (url.pathname === '/callback') {
      ...
      try {
        ...
        // 重定向到 CMS 同源的 callback.html（同源 postMessage 不会丢失）
