...
| 📱 **响应式布局** | 适配桌面端和移动端（含 Live2D 看板娘移动端适配） |
...
- **功能**：
  ...
  - AI 回复自动 TTS 朗读
  - 支持拖拽移动位置
- **移动端**：看板娘在 Wiki 聊天面板的 Live2D 区域内显示（高度 200px，模型尺寸 180px）
...
这是 v8.5 的已知 Bug，已在 v8.6 修复。关闭设置面板(✕)时未恢复 Live2D 区域。更新到最新代码即可。

### Q9: 手机上看不到 Live2D 看板娘？

v8.5 及之前版本在 `live2d.js` 中硬编码了 `screen.width >= 768` 限制，导致移动端跳过 Live2D 初始化。v8.6 已移除此限制，手机打开 Wiki 聊天面板即可看到看板娘。
