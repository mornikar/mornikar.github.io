...
class Live2DChat {
    ...
    constructor(config) {
        ...
        this.blogIndex = [];
        // 异步初始化
        console.log('[Live2DChat] 构造函数开始，configUrl=', this.configUrl);
        this.init().then(() => console.log('[Live2DChat] init() 完成')).catch(e => console.error('[Live2DChat] init 错误:', e));
    }
    ...
    applyConfig(cfg) {
        ...
        // ── TTS 状态（从 localStorage 恢复）──────────────────────
        const ttsStorage = JSON.parse(localStorage.getItem('waifu_tts_settings') || '{}');
        this.ttsEnabled = ttsStorage.enabled !== false; // 默认开
        this.ttsVoice = ttsStorage.voice || '';        // 选中的音色
        this.ttsVoicesLoaded = false;
    }
    ...
    // TTS：朗读文本（异步，等待 voices 列表加载完毕后再匹配音色）
    async speakText(text) {
        if (!this.ttsEnabled || !text) return;
        if (!window.speechSynthesis) return;
        console.log('[TTS] speakText 调用, ttsVoice:', this.ttsVoice, 'text:', text?.substring(0, 30));
        this._lastSpokenText = text;

        // 等待 voices 列表可用
        await this._waitForVoices();

        // 如果当前正在朗读，先取消并等待一小段延迟
        ...
        if (this.ttsVoice) {
            ...
            // 关键字匹配，优先选 Online/Natural 版本
            const keyword = this.ttsVoice;
            const matches = voices.filter(v =>
                (v.voiceURI && v.voiceURI.includes(keyword)) ||
                (v.name && v.name.includes(keyword))
            );
            // 优先级：Online (Natural) > Online > 其他
            let matched = matches.find(v => /online.*natural|natural.*online/i.test(v.voiceURI || v.name));
            if (!matched) matched = matches.find(v => /online/i.test(v.voiceURI || v.name));
            if (!matched && matches.length > 0) matched = matches[0];
            if (matched) {
                utter.voice = matched;
                console.log('[TTS] 匹配音色:', matched.voiceURI || matched.name);
            } else {
                console.warn('[TTS] 未匹配到音色:', keyword);
            }
        }

        // Chrome 长文本会被暂停，定期 resume 保活
        if (this._ttsKeepAlive) clearInterval(this._ttsKeepAlive);
        this._ttsKeepAlive = setInterval(() => {
            if (!speechSynthesis.speaking) { clearInterval(this._ttsKeepAlive); return; }
            speechSynthesis.resume();
        }, 5000);
        utter.onend = () => { clearInterval(this._ttsKeepAlive); this._updateTtsUI(false); };
        utter.onerror = (e) => {
            clearInterval(this._ttsKeepAlive);
            if (e.error !== 'interrupted' && e.error !== 'canceled') {
                console.error('[TTS] 播放错误:', e);
            }
            this._updateTtsUI(false);
        };
        this._updateTtsUI(true);
    ...
    initUI() {
        ...
        console.log('[Live2DChat] initUI HTML 注入，ttsEnabled=', this.ttsEnabled);
            ...
            <div id="waifu-chat-box">
                <div class="waifu-chat-header">
                    ...
                    <div class="waifu-chat-tools">
                        <span id="waifu-chat-clear" title="清空记忆">${svgTrash}</span>
                        <span id="waifu-chat-max-btn" title="最大化">${svgExpand}</span>
                        <span id="waifu-chat-close" title="关闭">${svgClose}</span>
                    </div>
                </div>
    ...
    // TTS 初始化：预加载 voices 列表
    _initTts() {
        if (!window.speechSynthesis) return;
        this._voicesReady = false;
        this._voicesReadyCallbacks = [];
        const loadVoices = () => {
            const voices = speechSynthesis.getVoices();
            if (voices.length > 0) {
                this.ttsVoicesLoaded = true;
                this._voicesReady = true;
                console.log('[TTS] 可用音色数:', voices.length);
                // 通知所有等待 voices 加载的回调
                this._voicesReadyCallbacks.forEach(cb => cb());
                this._voicesReadyCallbacks = [];
            }
        };
        // 首次尝试
        loadVoices();
        // 如果首次拿不到，监听 voiceschanged 事件
    ...
    _waitForVoices() {
        ...
        return new Promise(resolve => {
            this._voicesReadyCallbacks.push(resolve);
            // 超时兜底：2 秒后即使没加载完也继续
    ...
    initBoundsManagement() {
        // 防止重复注册监听器：只在首次调用时绑定，后续调用仅执行一次 checkBounds
        ...
        this._resizeHandler = () => this.checkBounds();
        window.addEventListener('resize', this._resizeHandler);

        // 使用 ResizeObserver 监听聊天窗口尺寸变化，兼容性较好的现代浏览器支持
        if (window.ResizeObserver) {
            this._resObserver = new ResizeObserver(() => this.checkBounds());
            this._resObserver.observe(this.chatBox);
        }
        // 监听聊天窗口样式变化，捕捉 display 变化导致的边界问题
        const waifuDOM = document.getElementById("waifu");
        if (waifuDOM) {
            this._mutObserver = new MutationObserver(() => this.checkBounds());
            this._mutObserver.observe(waifuDOM, { attributes: true, attributeFilter: ['style'] });
        }
    }
    ...
    async sendRequest(userText) {
        // 在用户手势上下文中激活 speechSynthesis，避免后续异步回调被 Chrome 静默阻止
        // 只需 warmup 一次即可，后续 Chrome 会记住已授权状态
        if (this.ttsEnabled && window.speechSynthesis && !this._ttsWarmedUp) {
            ...
            const typeWriter = setInterval(() => {
                ...
                if (charIndex >= fullAiReply.length) {
                    ...
                    const plainText = fullAiReply
                        ...
                        .replace(/\n+/g, ' ')
                        .trim();
                    console.log('[TTS] API回复完成，准备朗读，plainText长度:', plainText.length);
