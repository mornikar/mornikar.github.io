(() => {
    ...
    var v = class {
        ...
        async switchModel() {
            let o = parseInt(localStorage.getItem("modelId")) || 0;
            if (!this.modelList) await this.loadModelList();
    ...
    // TTS声音开关 SVG
    var svgTtsOn = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 256"><path d="M155.5 48.8v158.4l-58.2-44.7H56a8 8 0 0 1-8-8V101.7a8 8 0 0 1 8-8h41.3l58.2-44.9M208 128a79.9 79.9 0 0 1-56.6 75.1 8 8 0 0 1-3.6-15.5 64 64 0 0 0 45.2-60.2 64.4 64.4 0 0 0-18.2-45.4 8 8 0 1 1 12-10.6A80.3 80.3 0 0 1 208 128m-72 8a8 8 0 0 0-8 8v48a8 8 0 0 0 16 0v-48a8 8 0 0 0-8-8z"/></svg>';
    var svgTtsOff = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 256"><path d="M155.5 48.8v158.4l-58.2-44.7H56a8 8 0 0 1-8-8V101.7a8 8 0 0 1 8-8h41.3l58.2-44.9M208 128a79.9 79.9 0 0 1-56.6 75.1 8 8 0 0 1-3.6-15.5 64 64 0 0 0 45.2-60.2 64.4 64.4 0 0 0-18.2-45.4 8 8 0 1 1 12-10.6A80.3 80.3 0 0 1 208 128M178.3 45.7a8 8 0 0 1 0 11.3 80 80 0 0 1 0 113.4 8 8 0 0 1-11.3-11.3 64 64 0 0 0 0-90.8 8 8 0 0 1 11.3-11.3zM227.3 88.6a8 8 0 0 1 0 11.3L211.6 115a16 16 0 0 1-22.6-22.6l15.7-15.7a8 8 0 0 1 11.3 11.3zM228.7 128a8 8 0 0 1-11.3 11.3l-16-16a40 40 0 0 0-56.6 56.6l16 16a8 8 0 0 1-11.3 11.3l-16-16a56 56 0 0 1 0-79.2z"/></svg>';
    // 音色切换 SVG
    var svgVoice = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 256"><path d="M200.7 26.3a8 8 0 0 0-6.9-1.3l-22.1 5.9a48 48 0 0 0-37.4 37.4l-5.9 22.1a16 16 0 0 0 7 17.1l29.1 21.8a32 32 0 0 1 15.7 31v24.8a32 32 0 0 1-15.7 31l-29.1 21.8a16 16 0 0 0-7 17.1v9.8a16 16 0 0 0 16 16h9.8a16 16 0 0 0 17.1-7l21.8-29.1a32 32 0 0 1 31-15.7h24.8a32 32 0 0 1 31 15.7l21.8 29.1a16 16 0 0 0 17.1 7h9.8a16 16 0 0 0 16-16v-9.8a16 16 0 0 0-7-17.1l-29.1-21.8a32 32 0 0 1-15.7-31v-24.8a32 32 0 0 1 15.7-31l29.1-21.8a16 16 0 0 0 7-17.1v-9.8a16 16 0 0 0-16-16h-9.8a16 16 0 0 0-17.1 7l-21.8 29.1a32 32 0 0 1-31 15.7H174a32 32 0 0 1-31-15.7l-21.8-29.1a16 16 0 0 0-17.1-7h-9.8a16 16 0 0 0-16 16v9.8a16 16 0 0 0 6.4 12.8l31 23.3a8 8 0 0 1 0 12.9l-31 23.3a16 16 0 0 0-6.4 12.8v9.8a8 8 0 0 0 8 8h9.8a8 8 0 0 0 8.5-5.5l21.8-29.1a16 16 0 0 1 15.5-7.8h24.8a16 16 0 0 1 15.5 7.8l21.8 29.1a8 8 0 0 0 8.5 5.5h9.8a8 8 0 0 0 8-8v-9.8a16 16 0 0 0-6.4-12.8l-31-23.3a8 8 0 0 1 0-12.9l31-23.3a16 16 0 0 0 6.4-12.8v-9.8a8 8 0 0 0-8-8h-9.8a8 8 0 0 0-8.5 5.5l-21.8 29.1a16 16 0 0 1-15.5 7.8H174a16 16 0 0 1-15.5-7.8l-5.9-22.1a48 48 0 0 0 27.4-15.4l22.1 5.9a8 8 0 0 0 1.6.1z"/></svg>';
    ...
    var U = {
        ...
        "tts-toggle": {
            icon: svgTtsOn,
            callback: () => {
                const chat = window.live2dChatInstance;
                console.log('[TTS] 开关按钮点击, chat存在:', !!chat, '当前ttsEnabled:', chat?.ttsEnabled);
                if (chat) {
                    chat.ttsEnabled = !chat.ttsEnabled;
                    chat._saveTtsSettings();
                    if (!chat.ttsEnabled) chat.stopTts(); // 关闭时立即停止当前朗读
                    chat._updateTtsUI(false);
                    a(chat.ttsEnabled ? "TTS 已开启" : "TTS 已关闭", 2000, 9);
                }
            }
        },
        "tts-voice": {
            icon: svgVoice,
            callback: () => {
                console.log('[TTS] 音色按钮点击');
                // 切换音色选择面板的显示/隐藏
                let panel = document.getElementById('waifu-tts-voice-panel');
                if (panel) {
                    console.log('[TTS] 面板已存在，关闭');
                    panel.remove();
                    return;
                }
                const chat = window.live2dChatInstance;
                if (!chat) { console.log('[TTS] chat实例不存在'); return; }
                const VOICES = ['Xiaoxiao', 'Yunxi', 'Yunyang', 'Yunjian', 'Xiaoyi'];
                const LABELS = ['晓晓', '云希', '云扬', '云健', '小艺'];
                const voiceBtn = document.getElementById('waifu-tool-tts-voice');
                if (!voiceBtn) { console.log('[TTS] voiceBtn不存在'); return; }
                const rect = voiceBtn.getBoundingClientRect();
                const panelHeight = VOICES.length * 35 + 12; // 估算面板高度
                panel = document.createElement('div');
                panel.id = 'waifu-tts-voice-panel';
                // 面板往上展开：top 定位 = 按钮顶部 - 面板高度
                const panelTop = Math.max(4, rect.top - panelHeight);
                panel.style.cssText = `position:fixed;right:${window.innerWidth - rect.left + 8}px;top:${panelTop}px;background:rgba(30,30,40,0.95);border:1px solid rgba(100,160,255,0.3);border-radius:8px;padding:6px 0;z-index:99999;min-width:90px;box-shadow:0 4px 16px rgba(0,0,0,0.5);`;
                VOICES.forEach((v, i) => {
                    const item = document.createElement('div');
                    item.textContent = LABELS[i];
                    const isActive = chat.ttsVoice === v || chat.ttsVoice.includes(v);
                    item.style.cssText = `padding:6px 16px;cursor:pointer;font-size:13px;color:${isActive ? '#64a0ff' : '#ccc'};background:${isActive ? 'rgba(100,160,255,0.15)' : 'transparent'};text-align:center;transition:all 0.15s;`;
                    item.addEventListener('mouseenter', () => { item.style.background = 'rgba(100,160,255,0.25)'; });
                    item.addEventListener('mouseleave', () => { item.style.background = isActive ? 'rgba(100,160,255,0.15)' : 'transparent'; });
                    item.addEventListener('click', () => {
                        console.log('[TTS] 选择音色:', v, LABELS[i], 'ttsEnabled:', chat.ttsEnabled);
                        ...
                        chat._saveTtsSettings();
                        chat._updateTtsUI(false);
                        a(`音色已切换为「${LABELS[i]}」`, 2000, 9);
                        panel.remove();
                        // 直接在点击事件中朗读，不能延迟，否则 Chrome 会因用户手势策略阻止播放
                        if (chat.ttsEnabled) {
                            chat.speakText(`你好，我是${LABELS[i]}，音色切换成功！`);
                        }
                    });
                    panel.appendChild(item);
                });
                document.body.appendChild(panel);
                console.log('[TTS] 音色面板已创建, 当前音色:', chat.ttsVoice);
                // 点击面板外区域关闭
                const closePanel = (e) => {
                    if (!panel.contains(e.target) && e.target !== voiceBtn) {
                        panel.remove();
    ...
    function E(e) {
        ...
        (function () {
            ...
            // 初始化 TTS 开关按钮图标状态
            const ttsBtn = document.getElementById('waifu-tool-tts-toggle');
            if (ttsBtn) {
                try {
                    const ttsSettings = JSON.parse(localStorage.getItem('waifu_tts_settings') || '{}');
                    const enabled = ttsSettings.enabled !== false;
                    if (!enabled) {
                        ttsBtn.innerHTML = svgTtsOff;
